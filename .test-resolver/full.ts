 ...
 */
export function buildColumnMap(cells: string[]): ColumnMap | null {
  ...
  const findIdx = (...needles: string[]): number => {
    for (let i = 0; i < normalized.length; i++) {
      for (const needle of needles) {
        if (normalized[i] === needle) return i;
      }
    }
    return -1;
  };
 ...
 */
export function parseRowByHeaderMap(
  cells: string[],
  colMap: ColumnMap,
  rowIdx: number,
  activeAccountId: string
): Trade | null {
  if (!cells || cells.length < 8 || !colMap) return null;
  ...
  if (colMap.typeIdx >= 0 && colMap.typeIdx < cells.length) {
    ...
    if (t === "buy" || t === "خرید" || t === "long" || t === "in") orderType = "BUY";
    else if (t === "sell" || t === "فروش" || t === "short" || t === "out") orderType = "SELL";
  }
  if (!orderType) {
  ...
  // ------ Open / Close times
  let openTimeObj: { iso: string; timestamp: number } | null = null;
  let closeTimeObj: { iso: string; timestamp: number } | null = null;

  if (
    colMap.openTimeIdx >= 0 &&
  ...
  if (!openTimeObj) {
    ...
    cells.forEach((c, idx) => {
      ...
      if (resolved) dateAnchors.push({ ...resolved, cellIdx: idx });
    });
    if (dateAnchors.length > 0) openTimeObj = dateAnchors[0];
    if (dateAnchors.length > 1) closeTimeObj = dateAnchors[1];
  }
  if (!closeTimeObj) closeTimeObj = openTimeObj;
  if (!openTimeObj) return null;
  ...
  if (colMap.ticketIdx >= 0 && colMap.ticketIdx < cells.length) {
    ...
    if (cleaned) ticket = parseInt(cleaned, 10);
  }
  ...
  if (!ticket || Number.isNaN(ticket)) ticket = Date.now() + rowIdx;
  ...
  if (entryPrice === exitPrice && colMap.closeTimeIdx >= 0) {
    for (let i = colMap.closeTimeIdx + 1; i < cells.length; i++) {
      if (i === colMap.entryPriceIdx) continue;
  ...
  const durationMinutes = Math.max(
    1,
    Math.round((_closeTime.timestamp - openTimeObj!.timestamp) / 60000) || 15
  );
